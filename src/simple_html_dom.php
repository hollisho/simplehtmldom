...
namespace hollisho\simplehtmldom;

define('HO_HDOM_TYPE_ELEMENT', 1);
define('HO_HDOM_TYPE_COMMENT', 2);
define('HO_HDOM_TYPE_TEXT', 3);
define('HO_HDOM_TYPE_ENDTAG', 4);
define('HO_HDOM_TYPE_ROOT', 5);
define('HO_HDOM_TYPE_UNKNOWN', 6);
define('HO_HDOM_QUOTE_DOUBLE', 0);
define('HO_HDOM_QUOTE_SINGLE', 1);
define('HO_HDOM_QUOTE_NO', 3);
define('HO_HDOM_INFO_BEGIN', 0);
define('HO_HDOM_INFO_END', 1);
define('HO_HDOM_INFO_QUOTE', 2);
define('HO_HDOM_INFO_SPACE', 3);
define('HO_HDOM_INFO_TEXT', 4);
define('HO_HDOM_INFO_INNER', 5);
define('HO_HDOM_INFO_OUTER', 6);
define('HO_HDOM_INFO_ENDSPACE', 7);

defined('HO_DEFAULT_TARGET_CHARSET') || define('HO_DEFAULT_TARGET_CHARSET', 'UTF-8');
defined('HO_DEFAULT_BR_TEXT') || define('HO_DEFAULT_BR_TEXT', "\r\n");
defined('HO_DEFAULT_SPAN_TEXT') || define('HO_DEFAULT_SPAN_TEXT', ' ');
defined('HO_MAX_FILE_SIZE') || define('HO_MAX_FILE_SIZE', 600000);
define('HO_HDOM_SMARTY_AS_TEXT', 1);
...
function file_get_html(
	...
	$target_charset = HO_DEFAULT_TARGET_CHARSET,
	$stripRN = true,
	$defaultBRText = HO_DEFAULT_BR_TEXT,
	$defaultSpanText = HO_DEFAULT_SPAN_TEXT)
{
	if($maxLen <= 0) { $maxLen = HO_MAX_FILE_SIZE; }
	...
	$defaultSpanText = HO_DEFAULT_SPAN_TEXT)
{
	...
	if (empty($str) || strlen($str) > HO_MAX_FILE_SIZE) {
		$dom->clear();
		return false;
	}
...
{
	public $nodetype = HO_HDOM_TYPE_TEXT;
	public $tag = 'text';
	public $attr = array();
	...
	function dump_node($echo = true)
	{
		...
		if (isset($node->_[HO_HDOM_INFO_INNER])) {
			$string .= "'" . $node->_[HO_HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext()
	{
		if (isset($this->_[HO_HDOM_INFO_INNER])) {
			return $this->_[HO_HDOM_INFO_INNER];
		}

		if (isset($this->_[HO_HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[HO_HDOM_INFO_TEXT]);
		}
	...
	function outertext()
	{
		...
		if (isset($this->_[HO_HDOM_INFO_OUTER])) {
			return $this->_[HO_HDOM_INFO_OUTER];
		}
		...
		if ($this->dom && $this->dom->nodes[$this->_[HO_HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[HO_HDOM_INFO_BEGIN]]->makeup();
		}

		if (isset($this->_[HO_HDOM_INFO_INNER])) {
			// todo: <br> should either never have HO_HDOM_INFO_INNER or always
			if ($this->tag !== 'br') {
				$ret .= $this->_[HO_HDOM_INFO_INNER];
			}
		} elseif ($this->nodes) {
			foreach ($this->nodes as $n) {
		...
		if (isset($this->_[HO_HDOM_INFO_END]) && $this->_[HO_HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case HO_HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[HO_HDOM_INFO_TEXT]);
			case HO_HDOM_TYPE_COMMENT: return '';
			case HO_HDOM_TYPE_UNKNOWN: return '';
		}
		...
		// In rare cases, (always node type 1 or HO_HDOM_TYPE_ELEMENT - observed
		...
		if (!is_null($this->nodes)) {
			foreach ($this->nodes as $n) {
				...
				// after all.
				if ($n->tag === 'span') {
					$ret .= $this->dom->HO_DEFAULT_SPAN_TEXT;
				}
			}
		}
		return $ret;
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[HO_HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[HO_HDOM_INFO_QUOTE][$i])
				{
					case HO_HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case HO_HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}

				$ret .= $key
				. $this->_[HO_HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[HO_HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[HO_HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[HO_HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[HO_HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[HO_HDOM_INFO_END])) ? $this->_[HO_HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[HO_HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[HO_HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[HO_HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			if ($pass) $ret[$node->_[HO_HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[HO_HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[HO_HDOM_INFO_TEXT])) {
					return $this->_[HO_HDOM_INFO_TEXT] = $value;
				}
				return $this->_[HO_HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[HO_HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[HO_HDOM_INFO_QUOTE][] = HO_HDOM_QUOTE_DOUBLE;
		}
...
class simple_html_dom
{
		...
		$options = 0)
	{
		...
		if($options & HO_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[HO_HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
		...
		$defaultSpanText = HO_DEFAULT_SPAN_TEXT)
	{
		...
		$this->HO_DEFAULT_BR_TEXT = $defaultBRText;
		$this->HO_DEFAULT_SPAN_TEXT = $defaultSpanText;
		$this->root = new simple_html_dom_node($this);
		$this->root->tag = 'root';
		$this->root->_[HO_HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = HO_HDOM_TYPE_ROOT;
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					$this->parent->_[HO_HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag) !== $tag_lower) {
						...
						$this->parent->_[HO_HDOM_INFO_END] = $this->cursor;
					...
					$this->parent->_[HO_HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new simple_html_dom_node($this);
		$node->_[HO_HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0] === '!') {
			$node->_[HO_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
				$node->nodetype = HO_HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = HO_HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}

			if ($this->char === '>') { $node->_[HO_HDOM_INFO_TEXT] .= '>'; }
		...
		if (!preg_match('/^\w[\w:-]*$/', $tag)) {
			$node->_[HO_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = HO_HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do {
			...
			if ($this->pos >= $this->size - 1 && $this->char !== '>') {
				$node->nodetype = HO_HDOM_TYPE_TEXT;
				$node->_[HO_HDOM_INFO_END] = 0;
				$node->_[HO_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
				$node->tag = 'text';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[HO_HDOM_INFO_END] = 0;
				$node->_[HO_HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->char === '=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[HO_HDOM_INFO_QUOTE][] = HO_HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char != '>') { $this->char = $this->doc[--$this->pos]; } // prev
				}

				$node->_[HO_HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[HO_HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>') === '/') {
			$node->_[HO_HDOM_INFO_ENDSPACE] .= '/';
			$node->_[HO_HDOM_INFO_END] = 0;
		} else {
			// reset parent
		...
		if ($node->tag === 'br') {
			$node->_[HO_HDOM_INFO_INNER] = $this->HO_DEFAULT_BR_TEXT;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"':
				$quote_type = HO_HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('"');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'':
				$quote_type = HO_HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('\'');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			default:
				$quote_type = HO_HDOM_QUOTE_NO;
		...
		if (!$is_duplicate) {
			$node->_[HO_HDOM_INFO_QUOTE][] = $quote_type;
